...
- (IBAction)foucsChange:(UISlider *)sender {
    ...
    [captureDevice setFocusModeLockedWithLensPosition:sender.value completionHandler:nil];
    [captureDevice unlockForConfiguration];
...
{
    AVCaptureDevice *captureDevice = [self.mediaDeviceInput device];
    CGFloat maxZoomFactor = captureDevice.activeFormat.videoMaxZoomFactor;
    if (@available(iOS 11.0, *)) {
